define(function(require) {
	...
	return function() { //mange and distribution
		...
		var chart = function(selector) {
			...
			var chartContainer = d3.select(selector).append(opts.domType)
				.attr({
			...
			plotGenerator(chartContainer).then(function(plotObj) {
				...
				if(opts.tooltip) {
					...
					var tooltipObj;
					tooltipGenerator().then(function(result) {
						tooltipObj = result;
					});
				}
				...
				defer.resolve(plotObj);
			});

			//console.log(plotOpts, legendOpts);

			//var legendGenerator = legendService().options(opts.legend)
