define(function(require) {
	...
	/**
	   ...
	   */
	return function() { //create chart maker
		var _opts = {};
		...
		var chartMaker = function(container) { //create chart object
			...
			var getLoadList = function(opts) {
				var list = [];
				if(opts.axis) {
					list.push('../scale/scale');
					var rulers = _.filter(opts.axis.data, function(d) {
						return !!d.show;
					});
					if(rulers.length) {
						list.push('../axis/axis');
			...
			var loadList = getLoadList(_opts);

			if(loadList.length) {
				require.async(loadList, function(scale, axis) {
				...
				_opts.series = _.merge(_opts.series, _opts.data);
				chartObj = makeChart();
				defer.resolve(chartObj);
			}
